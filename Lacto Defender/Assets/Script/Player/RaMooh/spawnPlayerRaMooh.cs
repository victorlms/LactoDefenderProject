...
public class spawnPlayerRaMooh : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D other){
		...
		if (spawn == false && gameObject.GetComponent<movimentoRaMooh> ().walking == true) {

			if (gameObject.GetComponent<movimentoRaMooh> ().field.gameObject == other.gameObject) {
				posiciona = true;
			}
...
*/
			...
			/*if (gameObject.transform.GetComponent<movimentoRaMooh> ().walking) {
				if (onField == false && posiciona == true && other.gameObject == objeto[1].gameObject) {

					gameObject.transform.position = other.gameObject.transform.position;
					gameObject.transform.GetComponent<movimentoRaMooh> ().walking = false;
		...
		if (spawn == false && other.tag == "Box") {
			...
			if (gameObject.transform.GetComponent<movimentoRaMooh> ().walking) {
				if (onField == false 
					&& posiciona == true 
					&& other.gameObject == gameObject.transform.GetComponent<movimentoRaMooh>().objeto[1].gameObject) {
